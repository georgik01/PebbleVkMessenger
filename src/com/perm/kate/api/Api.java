...
public class Api {
    ...
    private JSONObject sendRequestForLongPoll(String key, String server, long ts) throws IOException, JSONException, KException {
    	String s = String.valueOf(ts);
    	String url = "https://"+server+"?act=a_check&key="+key+"&ts="+s+"&wait=25&mode=2";
        String body="";
        Log.i(TAG, "url="+url);
        if(body.length()!=0)
            Log.i(TAG, "body="+body);
  ...
  //http://vk.com/dev/users.get
    public JSONArray getProfile(String uid) throws IOException, JSONException, KException{
        if (uid == null)
            return null;
        Params params = new Params("users.get");
        if (uid != null)
  ...
  //http://vk.com/dev/messages.getLongPollServer
    public Object[] longPollServer(String key, String server, long ts) throws IOException, JSONException, KException{
        //Params params = new Params("messages.getLongPollServer");
        //addCaptchaParams(captcha_key, captcha_sid, params);
        JSONObject root = sendRequestForLongPoll(key,server,ts);
        JSONObject response = root.getJSONObject("response");
        String newTs=response.getString("ts");
        String updates=response.getString("updates");
        return new Object[]{ts, updates};
    }
